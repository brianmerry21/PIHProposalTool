...
def extract_total_value(text):
    ...
    try:
        ...
        # First look for TOTAL INVESTMENT pattern specific to Modula PDFs (from screenshot)
        total_investment_pattern = r"TOTAL\s+INVESTMENT:?\s*\$?\s*(\d{1,3}(?:,\d{3})*\.\d+)"
        investment_match = re.search(total_investment_pattern, text, re.IGNORECASE | re.DOTALL)
        if investment_match:
            total_value = investment_match.group(1)
            # Convert to float and round to 2 decimal places
            total_value_float = round(float(total_value.replace(',', '')), 2)
            total_value = f"{total_value_float:.2f}"
            logger.info(f"Found total investment value: ${total_value}")
            return f"${total_value}", total_value_float
        
        # Standard total patterns if TOTAL INVESTMENT is not found
        total_patterns = [
        ...
        if all_amounts:
            ...
            try:
                ...
                logger.info(f"Found largest amount: ${largest_amount_str}")
                return f"${largest_amount_str}", largest_amount
            except:
                # Or just use the last amount as a fallback
                last_amount = all_amounts[-1]
                last_amount_float = float(last_amount.replace(',', ''))
                logger.info(f"Using last amount as total: ${last_amount}")
                return f"${last_amount}", last_amount_float
        
        logger.warning("Could not find total value in PDF")
        return "$0.00", 0.0  # Default if no match found
...
def process_pdf_to_excel(pdf_path, excel_path, template_path=None, customer_info=None):
    ...
    try:
        ...
        # If no tables were found, extract text and convert to structured format
        if not tables:
        ...
        if template_path and os.path.exists(template_path):
            # Load the template workbook to preserve formulas
            ...
            for i, table in enumerate(tables):
                ...
                # Write table values to cells (overwrite only values, keep formulas)
                for r_idx, row in enumerate(table.values, start=1):
                    for c_idx, value in enumerate(row, start=1):
                        ws.cell(row=r_idx, column=c_idx, value=value)
            
            wb.save(excel_path)
        else:
            # No template: just write tables to Excel
...
def create_modula_mapping_rules():
    ...
    mapping_rules = {
    'Primary Option': {
        'F1': {'default': 'Dart Aerospace'},
        'B7': {
            'pattern': r'Model\s+(ML25)',
            'group': 1,
            'transform': lambda x: x.strip()
        },
        'G7': {'default': '$96332.45'},
        'B8': {
            'pattern': r'Ref\. Offer No\.: QLI-(\d+)',
            'group': 1,
            'transform': lambda x: x.strip()
        },
        'B9': {
        ...
        'B10': {
            'custom_handler': lambda text: "Width x Depth Placeholder"
        },
...
def extract_customer_info_from_pdf(pdf_text):
    ...
    customer_pattern = r"(.*?)\s*(?:Scientific|Inc\.?|Corp\.?|Corporation|Aerospace|Company|LLC)\s+"
